...
def preprocess(image):
    with tf.name_scope("preprocess"): #定义命名空间 tf.Variable
        # [0, 1] => [-1, 1]
        return image * 2 - 1


def deprocess(image):
    with tf.name_scope("deprocess"):#定义命名空间 tf.Variable
        # [-1, 1] => [0, 1]
        return (image + 1) / 2
...
def batchnorm(input):
    with tf.variable_scope("batchnorm"):
        ...
        mean, variance = tf.nn.moments(input, axes=[0, 1, 2], keep_dims=False)  #返回向量大的均值和方差
        variance_epsilon = 1e-5
        # 批标准化bn，一般用在激活函数之前，使用x=Wx+b各个维度均值为0,方差为1通过规范化让激活函数分布在线性区间，让一层的输入有一个稳定的分布会有利于网络训练
        # 优点 加大探索步长，加快收敛速度
        # 更容易跳出局部极小
...
def rgb_to_lab(srgb):
    with tf.name_scope("rgb_to_lab"):
        ...
        with tf.name_scope("xyz_to_cielab"):
            ...
            # （这里指multiply这个函数实现的是元素级别的相乘，也就是两个相乘的数元素各自相乘，而不是矩阵乘法，注意和tf.matmul区别）
...
def lab_to_rgb(lab):
    with tf.name_scope("lab_to_rgb"):
        lab = check_image(lab)
        lab_pixels = tf.reshape(lab, [-1, 3]) #转化为新shape，若有一个维度设置为-1,会自动推倒
        ...
        return tf.reshape(srgb_pixels, tf.shape(lab)) #转化为新shape，若有一个维度设置为-1,会自动推倒
...
def load_examples():
    ...
    with tf.name_scope("load_images"):
        # 提供的可配置参数来设置文件名乱序和最大的训练迭代数， QueueRunner会为每次迭代(epoch)将所有的文件名加入文件名队列中， 如果shuffle=True的话， 会对文件名进行乱序处理。这一过程是比较均匀的，因此它可以产生均衡的文件名队列。
        ...
        # 转换数据类型
        # 因为为了将图片数据能够保存到 TFRecord 结构体中，所以需要将其图片矩阵转换成 string，
        ...
        if a.lab_colorization:
            ...
            # 会向tensor中插入一个维度，插入位置就是参数代表的位置（维度从0开始）。
    ...
    def transform(image):
        ...
        # 第二个参数改变之后图像的大小，method用于表示改变图像过程用的差值方法。
...
def main():
    ...
    if a.mode == "export":
        ...
        # 转换数据类型
        # 为了将图片数据能够保存到TFRecord结构体中,所以需要将其图片矩阵转换成string,
        # 所以为了在使用时能够转换回来,这里确定下数据格式为 tf.float32
    ...
    with tf.name_scope("encode_images"):
        display_fetches = {
            ...
            # 接受一个函数对象，然后用该函数对象对集合（elems）中的每一个元素分别处理，
        ...
        if a.mode == "test":
            ...
            for step in range(max_steps):
                ...
                if should(a.trace_freq):
                    ...
                    # 定义TensorFlow运行的元信息，这样可以记录训练时运算时间和内存占用等方面的信息。
